/*
  verify-3.7-2026-08-30.js - sixth independent machine pass on item 3.7
  (Smartts Chemist, Bootle, single branch).
  ...
  Legs 1-11 follow the single-branch template proved on 3.5 (counts,
  sheet agreement and one-label-per-block, own town, service words,
  cross-town absence, phone/postcode isolation, lengths, JSON-LD field
  by field plus tel:, foreign widget ids, foreign brand labels, banner).
  Note SK Chemists shares the town Bootle, so the cross-town leg
  excludes the shared seoTown by value while the phone/postcode leg
  still isolates the two Bootle branches from each other.

  NEW LEGS THIS PASS:
   12. ROOT ATTRIBUTES (from the 3.6 template): every service page's
       #rbhsv-root carries data-branch equal to branchName or brandLabel
       and a non-empty data-service.
   13. FRAGMENT RESOLUTION, independent re-proof of the fifth pass fix
       (a7fa444): every href="#x" on every page resolves to an id="x"
       in the same page.
   14. NO HARD-CODED WIDGET ID: the pages carry the comment "do not
       hard-code a widgetId"; this leg enforces it - no 24-hex string
       anywhere in any Smartts page. A widget id belonging to NO branch
       would pass leg 9 (which only bars other branches' ids), so this
       closes that gap for the whole family.
   15. WHATSAPP AGREEMENT: all 12 pages carry the same data-wa, it is
       447 plus nine digits, and it equals the DEFAULT_WHATSAPP read as
       text from modules/service/service.js and modules/switch/switch.js.
*/
"use strict";
const fs = require("fs");
...
const sm = data.branches.find(b => b.id === "smartts_bootle");
if (!sm) { console.log("FATAL: smartts_bootle not in branches.json"); process.exit(1); }
...
["modules/service/pages", "modules/switch/pages"].forEach(d => {
  fs.readdirSync(path.join(REPO, d)).forEach(f => {
    if (f.endsWith(".html") && /smartts/.test(f)) PAGES.push(d + "/" + f);
  });
});
...
function serviceWordsFor(file) {
  ...
  const m = /^([a-z-]+)-treatment-smartts/.exec(base);
...
PAGES.forEach(relFile => {
  ...
  // 5. CROSS-TOWN (SK Chemists also carries seoTown Bootle; shared value excluded)
  ...
  // 6. PHONES AND POSTCODES (includes isolation from SK Chemists Bootle)
...
// 11. BANNER
const bf = path.join(REPO, "modules/switch/pages/banners/switch-prescriptions-smartts-bootle.txt");
check(fs.existsSync(bf), "banner switch-prescriptions-smartts-bootle.txt missing");
if (fs.existsSync(bf)) {
  const txt = fs.readFileSync(bf, "utf8");
  check(txt.indexOf("switch-prescriptions-smartts-bootle") !== -1, "banner: does not point at Smartts' own switch page");
}

console.log("verify-3.7: " + pagesChecked + " Smartts pages checked, " + Object.keys(sheetByPermalink).length + " sheet permalinks parsed, " + checksRun + " checks run");
if (fails.length) {
  console.log("FAILURES (" + fails.length + "):");
  fails.forEach(f => console.log("  FAIL  " + f));
  process.exit(1);
}
console.log("CLEAN on all legs: counts, sheet agreement and one-label-per-block, own town, service words, cross-town absence, phone/postcode isolation (including from SK Chemists Bootle), lengths, JSON-LD field-by-field, tel:, widget isolation, brand isolation, banner, root attributes, fragment resolution, no hard-coded widget ids, WhatsApp agreement.");
