/*
  verify-3.4-2026-09-01-eighth.js - eighth independent machine pass on item
  3.4 (Cherry Lane Pharmacy, Walton, single branch).

  Imports NOTHING from tools/. Own regexes, own service-word table, own
  sheet-parsing and cross-town logic, written fresh for this run rather than
  copied from the seventh pass's script. Reads branches.json as data only.

  Repeats every invariant the seventh pass (2026-08-31) proved for the 12
  Cherry Lane pages (11 service, 1 switch), plus two checks that pass did
  not carry:
    - the two weebly-paste replacement blocks' postal address string, since
      item 1.4/3.4-fourth's own citation-consistency fault (seoTown leaking
      into a hand-typed postal address) is exactly the class of defect a
      generator-only diff cannot see;
    - a direct read of check-service-links.js's newly widened JS-injected
      copy scan (landed this run, item 6.2 fifth pass) to confirm it still
      resolves modules/service/service.js and modules/switch/switch.js
      against Cherry Lane's own pharmacy-first and switch pages rather than
      silently skipping them.

  Checks, per Cherry Lane page:
    ...
    5. No other live branch's seoTown anywhere, serviceAreaList excuse applied.
   ...
   10. data-wa="<cl.whatsapp>" on every page with a module root, no foreign
       whatsapp number anywhere.
   11. The switch banner points at Cherry Lane's own switch page.
   12. The two weebly-paste replacement blocks name no POM, link their
       destination page, and do NOT put Walton inside the postal address
       string (the fourth-pass fault, re-tested every pass since as a
       standing regression guard).
   13. NEW: modules/service/service.js and modules/switch/switch.js are
       readable and are the files check-service-links.js actually scanned
       this run (cross-checked against its own EXTRA_FILES/JS-scan list).
*/
"use strict";
const fs = require("fs");
...
const OWN_POSTAL = (cl.streetAddress + ", " + cl.addressLocality + " " + cl.postalCode).toLowerCase();
[
  ...
  // NEW (13): the citation-consistency regression guard. The fourth pass's
  // real defect was the seoTown "Walton" typed INSIDE the postal address
  // string, between street and postal town. Re-tested every pass since as
  // a positive check that the postal address string is exactly the postal
  // form (no seoTown inserted) - not just an absence-of-Walton scan, which
  // would miss a different wrong word inserted in the same spot.
  const addrLine = html.replace(/\s+/g, " ");
  check(addrLine.indexOf(OWN_POSTAL) !== -1,
    rel + ": postal address string does not read '" + OWN_POSTAL + "' verbatim (citation-consistency regression guard, item 1.4/3.4-fourth)");
});

// NEW (14): confirm check-service-links.js's JS-injected-copy widening
// (landed this run as item 6.2 fifth pass) actually names the two files
// its own header says it now reads, so this item's Pharmacy First and
// switch pages are inside that scan rather than silently excluded.
const csl = fs.readFileSync(path.join(REPO, "tools/check-service-links.js"), "utf8");
["modules/service/service.js", "modules/switch/switch.js"].forEach(f => {
  check(csl.indexOf(f) !== -1, "check-service-links.js does not reference " + f + " (expected after item 6.2 fifth pass widening)");
});

console.log("verify-3.4-eighth: " + pagesChecked + " Cherry Lane pages checked, " + checksRun + " checks run");
if (fails.length) {
  console.log("FAILURES (" + fails.length + "):");
  fails.forEach(f => console.log("  FAIL  " + f));
  process.exit(1);
}
console.log("CLEAN on all legs including the citation-consistency regression guard and the check-service-links.js widening confirmation.");
